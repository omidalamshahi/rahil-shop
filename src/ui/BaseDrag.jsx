...
1.if not no need to scroll remove the functionality(cursor change,backdrop,...)
3.ability to add margin(some space) for scrollbar(addition to two version which exist)
4.fix: scrollbar on mobile is not functional
5.add: scrollbar position to left,right,top,bottom of the baseDrag 
6.add: scrollbar always stay opacity:low(no hover effect)
7.set max and min position for scrollbar
...
import classes from './css/BaseDrag.module.css';
...
const BaseDrag = ({
  ...
  const handleDragEnd = (e) => {
    ...
    sliderRef.current.style.pointerEvents = '';
  ...
  const SCROLL_ACCELERATION = 1 * scrollMultiplication; // Speed multiplier base on mouse movement
  let MARGIN_LIMIT = 7; // limits to mouse movement before scrolling
  const handleDrag = (e) => {
    ...
    //skip draging while holding on to draggable item
    if (
      tag === 'input' ||
      tag === 'textarea' ||
      tag === 'select' ||
  ...
  const handleScrollTrough = (e) => {
    ...
    animateScroll(sliderScrollPos, newScrollPos, 500, easeInOut);
  };
    ...
    const step = (timestamp) => {
      ...
      if (xAxios) {
        setScrollPosX((prev) => {
          ...
          if (Math.abs(prev - newPosition) < 0.01) {
            stopAnimation();
            return prev;
          }
          return newPosition;
  ...
  useEffect(() => {
    ...
    const sliderOffset = xAxios ? element.offsetWidth : element.offsetHeight;
    const sliderScrollLength = xAxios
      ? element.scrollWidth
